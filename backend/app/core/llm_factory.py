...
class FallbackOfflineChatModel(BaseChatModel):
    ...
    def _generate(self, messages: List[BaseMessage], stop: Optional[List[str]] = None, **kwargs: Any) -> Any:
        ...
        if time_match and ("deliveries" in last_msg_lower or "delivery" in last_msg_lower or "orders" in last_msg_lower):
            ...
            response_text = f"```sql\nSELECT count(*) AS completed_deliveries FROM delivery_orders WHERE status = 'completed' AND delivery_date >= date('now', '-{days} days');\n```"
        elif "delivered the most gallons of diesel last month" in last_msg_lower:
            response_text = "```sql\nSELECT tenant_id, sum(gallons_delivered) AS total_diesel_gallons FROM delivery_orders WHERE product_type = 'diesel' AND status = 'completed' AND strftime('%Y-%m', delivery_date) = strftime('%Y-%m', date('now', 'start of month', '-1 month')) GROUP BY tenant_id ORDER BY total_diesel_gallons DESC LIMIT 1;\n```"
            ...
            response_text = "```sql\nSELECT count(*) AS emergency_orders_count FROM delivery_orders WHERE tenant_id = 4 AND priority = 'emergency' AND order_date >= date('now', '-30 days');\n```"
            ...
            response_text = "```sql\nWITH last_30 AS (SELECT tenant_id, count(*) AS count_last_30 FROM delivery_orders WHERE status = 'completed' AND delivery_date > date('now', '-30 days') AND delivery_date <= date('now') GROUP BY tenant_id), prev_30 AS (SELECT tenant_id, count(*) AS count_prev_30 FROM delivery_orders WHERE status = 'completed' AND delivery_date > date('now', '-60 days') AND delivery_date <= date('now', '-30 days') GROUP BY tenant_id) SELECT p.tenant_id, p.count_prev_30, l.count_last_30, (l.count_last_30 - p.count_prev_30) AS change FROM prev_30 p JOIN last_30 l ON p.tenant_id = l.tenant_id WHERE l.count_last_30 < p.count_prev_30 ORDER BY change ASC;\n```"
        else:
